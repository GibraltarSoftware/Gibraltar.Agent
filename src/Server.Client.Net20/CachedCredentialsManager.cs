...
namespace Gibraltar.Server.Client
{
    ...
    public static class CachedCredentialsManager
    {
        ...
        private static IWebAuthenticationProvider g_DefaultAuthenticationProvider; //PROTECTED BY LOCK
        ...
        /// <summary>
        /// Optional.  A default authentication provider to use for all requests
        /// </summary>
        public static IWebAuthenticationProvider DefaultAuthenticationProvider
        {
            get
            {
                lock (g_Lock)
                {
                    return g_DefaultAuthenticationProvider;
                }
            }
            set
            {
                lock (g_Lock)
                {
                    g_DefaultAuthenticationProvider = value;
                }
            }
        }
        ...
        {
            //a *little* treachery here: We want all of our cluster elements to be considered the same since they share creds.
            var normalizedHostname = hostName.ToLowerInvariant(); //hopefully that doesn't mess up Unicode host names...

            switch (normalizedHostname)
            {
                //these are our two back-ends for Loupe.
                case "us.onloupe.com":
                case "europe.onloupe.com":
                    return HubConnection.LoupeServiceServerName.ToLowerInvariant();
                default:
        ...
        private static IWebAuthenticationProvider GetCachedCredentials(string entryUri, bool useApiKey, Guid repositoryId, string keyContainerName, bool useMachineStore)
        {
            ...
            lock (g_Lock) //gotta be MT safe!
            {
                ...
                if (g_CachedCredentials.TryGetValue(cacheKey, out credentials) == false)
                {
                    ...
                    else if (g_DefaultAuthenticationProvider != null)
                    {
                        //since they provided a default authentication provider (this is really just for Agent scenarios)
                        //we'll use that, but *not* add it to cache so the cache can override us.
                        credentials = g_DefaultAuthenticationProvider;
                    }
                }
                        ...
                        else
                        {
                            if (credentialEventArgs.AuthenticationProvider == null)
                            {
                                throw new InvalidOperationException(
                                    "No credentials are available for the specified server");
                            }
