...
namespace Gibraltar.Messaging
{
    ...
    /// </summary>
    public class FileMessengerConfiguration : MessengerConfiguration
    {
        ...
        internal FileMessengerConfiguration(string name, SessionFileElement configuration, ServerConfiguration server)
            : base(name, typeof(FileMessenger).AssemblyQualifiedName)
        {
            Server = server;
        ...
        /// </summary>
        internal FileMessengerConfiguration(string name, XmlNode gibraltarNode, ServerConfiguration server) 
            : base(name, typeof(FileMessenger).AssemblyQualifiedName)
        {
            ...
            XmlNode node = gibraltarNode.SelectSingleNode("sessionFile");
            ...
            if (node != null)
            {
                ...
                IndexUpdateInterval = AgentConfiguration.ReadValue(node, "indexUpdateInterval", baseline.IndexUpdateInterval);
        ...
        public void Save(XmlNode gibraltarNode)
        {
            ...
            AgentConfiguration.WriteValue(newNode, "indexUpdateInterval", IndexUpdateInterval, baseline.IndexUpdateInterval);
        ...
        private void Initialize(SessionFileElement configuration)
        {
            //copy the configuration from the log element
            Folder = configuration.Folder;
            AutoFlushInterval = configuration.AutoFlushInterval;
            IndexUpdateInterval = configuration.IndexUpdateInterval;
