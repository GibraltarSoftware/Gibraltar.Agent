...
namespace Gibraltar.Messaging.Net
{
    ...
    public abstract class NetworkClient : IDisposable
    {
        ...
        private Thread m_BackgroundReader;
            ...
            try
            {
                ...
                while (!m_Closed)
                {
                    try
                    {
                        ...
                        using (TcpClient tcpClient = GetTcpClient())
                        {
                            m_TcpClient = tcpClient;

                            CalculateStateMessage(tcpClient);
                            Stream tcpStream;
                            NetworkStream rawStream = tcpClient.GetStream();
                            if ((m_Options != null) && (m_Options.UseSsl))
                            {
                                try
                                {
                                    ...
                                    sslStream.AuthenticateAsClient(m_Options.HostName, null, sslProtocols, true);
                                    tcpStream = sslStream;
                                }
                    ...
                    {
                        //in this case we're existing hard - don't log, just go.
                        throw;
                    }
        ...
        private void Initialize()
        {
            lock(m_Lock)
            {
                ...
                m_BackgroundReader = new Thread(AsyncNetworkStreamMain);
                m_BackgroundReader.IsBackground = true;
                m_BackgroundReader.Name = "Loupe Tcp Network Client";
        ...
        private TcpClient GetTcpClient()
        {
            ...
            else
            {
                try
                {
                    newClient = new TcpClient(m_Options.HostName, m_Options.Port);
                }
                catch (SocketException ex)
                {
                    innerException = ex;
        ...
        #region Private Class AsyncSocketReadState

        private class AsyncSocketReadState
        {
            public AsyncSocketReadState(Stream stream)
            {
                Stream = stream;
