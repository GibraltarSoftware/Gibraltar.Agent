...
namespace Gibraltar.Messaging
{
    internal class FileMessenger : MessengerBase
    {
        ...
        protected override void OnInitialize(MessengerConfiguration configuration)
        {
            ...
            FileMessengerConfiguration fileConfiguration = (FileMessengerConfiguration)configuration;
        ...
        protected override void OnExit()
        {
            ...
            if (m_CurrentSerializer != null)
            {
                ...
                m_CurrentSerializer.Synchronous = true;
            }
        }
        ...
        protected override void OnFlush(ref MaintenanceModeRequest maintenanceRequested)
        {
            ...
            if (m_CurrentFile != null && m_CurrentFile.Length > m_MaxFileSizeBytes)
            {
                maintenanceRequested = MaintenanceModeRequest.Regular;
            }
            else if (DateTime.Now > m_FileExpiration)
        ...
        private void GetSessionFileLock()
        {
            ...
            Guid sessionId = Log.SessionSummary.Id;
            string sessionLockName = sessionId.ToString();
            m_SessionFileLock = InterprocessLockManager.Lock(this, m_SessionLockFolder, sessionLockName, 0, true);

            if (m_SessionFileLock == null)
            {
                Log.Write(LogMessageSeverity.Information, "Gibraltar.Agent.FileMessenger", "Gibraltar Agent unable to get the unique lock for a new session",
                    "The Gibraltar Agent's FileMessenger was not able to lock this active session as Running.  "+
        ...
        private void ReleaseSessionFileLock()
        {
            ...
            {
                Log.Write(LogMessageSeverity.Information, LogWriteMode.Queued, ex, "Gibraltar.Agent.FileMessenger", "Gibraltar Agent got an error while releasing the unique lock for this session",
                    "The Gibraltar Agent's FileMessenger was not able to properly release the lock as this session exits.  "+
