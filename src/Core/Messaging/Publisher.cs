    ...
    internal class Publisher
    {
        ...
        private void CreateMessageDispatchThread()
        {
            lock (m_MessageDispatchThreadLock)
            {
                ...
                m_MessageDispatchThread.TrySetApartmentState(ApartmentState.MTA);
                // We generally WANT to keep the OS alive as a foreground thread so we make sure logs get flushed.
        ...
        private void MessageDispatchMain()
        {
            ...
            catch (Exception ex)
            {
                ...
                // We get a ThreadAbortException if we're a background thread and the app domain is exiting.
                // That's not an error, so we need to make sure anyone waiting can proceed and exit as well.
        ...
        private void DispatchPacket(PacketEnvelope envelope)
        {
            ...
            lock (envelope)
            {
                ...
                else
                {
                    ...
                    {
                        if (ex is ThreadAbortException)
                            throw; // Uh-oh, we need to abort the thread to exit for real, throw it on out.
        ...
        private void EnsureInitialized()
        {
            if (m_Initialized == false)
            {
                ...
                try
                {
                    if (m_Configuration.Viewer.Enabled)
                    {
                        // Only add the viewer messenger if we're in an app type that can use it (especially not in a service).
                        // There's no need to waste all that extra memory.
                        ApplicationType appType = m_SessionSummary.AgentAppType;
                        if (appType == ApplicationType.Windows)
                            AddMessenger(m_Configuration.Viewer);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void AddMessenger(MessengerConfiguration configuration)
        {
            IMessenger newMessenger = null;
            try
            {
                //see what kind of type information we got
                Type messengerType = Type.GetType(configuration.MessengerType, true, true);
