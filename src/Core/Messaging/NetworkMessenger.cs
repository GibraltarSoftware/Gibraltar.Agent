...
namespace Gibraltar.Messaging
{
    internal class NetworkMessenger : MessengerBase
    {
        ...
        private volatile bool m_ActiveRemoteConnectionAttempt; //for making sure we only do one hub connection attempt at a time
        ...
        public NetworkMessenger()
            : base(false)
        {
            //we're just here so we can tell the base that we don't support write through behavior
        }
        ...
        public void SendToServer(SendSessionCommandMessage sendSessionCommand)
        {
            try
            {
                if (Log.SendSessions(sendSessionCommand.Criteria, null, true) == false) //we love async!
                {
        ...
        protected override void OnInitialize(MessengerConfiguration configuration)
        {
            ...
            NetworkMessengerConfiguration messengerConfiguration = (NetworkMessengerConfiguration)configuration;
            ...
            if (messengerConfiguration.AllowRemoteClients)
            {
                ...
                ServerConfiguration server = Log.Configuration.Server;
        ...
        private void AttemptRemoteConnectionAsync()
        {
            ...
            ThreadPool.QueueUserWorkItem(AsyncEnsureRemoteConnection);
        }
        ...
        private void AsyncEnsureRemoteConnection(object state)
        {
            try
            {
                try
                {
                    ...
                    if ((m_HubConnection.IsConnected == false) || (hubConfigurationExpiration < DateTimeOffset.Now))
                    {
                        ...
                        if (m_HubConnection.Status == HubStatus.Expired)
                        {
                            ...
                            {
                                TimeSpan expiredTimeframe = connectionAttemptTime - m_HubConnection.ExpirationDt.Value;
                        ...
                        else
                        {
                            ...
                            newLiveStreamOptions = m_HubConnection.AgentLiveStreamOptions;
