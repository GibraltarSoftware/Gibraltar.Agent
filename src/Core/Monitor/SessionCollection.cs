    ...
    public class SessionCollection : IList<Session>
    {
        ...
        public Session Add(Stream fileStream, bool useOriginalStream)
        {
            ...
            if (useOriginalStream == false)
            {
                ...
                newStream.Position = originalPosition; // Since we copied the whole source stream we need to move to whatever offset we originally started from

                fileStream.Position = originalPosition; // And set it back where it had been before we copied it.
            }
            else
            {
        ...
        public bool Remove(Guid sessionId)
        {
            ...
            lock (m_Lock)
            {
                if (TryGetValue(sessionId, out Session victim))
                {
                    //just try to remove it from the two collections, you never know.
                    itemRemoved = m_SessionsList.Remove(victim);
                    itemRemoved |= m_SessionsById.Remove(sessionId);
                }
            }
        ...
        public bool Remove(Session item)
        {
            ...
            {
                //just try to remove it from the two collections, you never know.
                itemRemoved = m_SessionsList.Remove(item);
                itemRemoved |= m_SessionsById.Remove(item.Id);
            }
