...
namespace Gibraltar.Monitor
{
    ...
    public class Session : IDisplayable, IComparable<Session>, IEquatable<Session>, ISessionPacketCache, IDisposable
    {
        ...
        private Analysis m_Analysis; // our default analysis (intrinsic to the session)
        ...
        internal void LoadPacketStream(GLFReader glfReader)
        {
            ...
            analysisPacketFactory.Register(packetReader);
            ...
            while ((stream = packetManager.GetNextPacket()) != null)
            {
                ...
                if (nextPacket != null)
                {
                    try
                    {
                        ...
                        else if ((analysis = nextPacket as Analysis) != null)
                        {
                            //Analysis is his own animal because it can be persisted as part of a session or independently.
                            m_Analysis = analysis;
                        }
                        else
                        {
        ...
        /// <summary>
        /// The default analysis for this session.  There is always a default analysis, however it may be empty.
        /// </summary>
        public Analysis Analysis
        {
            get 
            { 
                //There is always a session analysis.  If we don't have one yet, make it
                if (m_Analysis == null)
                {
                    m_Analysis = new Analysis();
                }
        ...
        public void Write(Stream stream, int majorVersion, int minorVersion)
        {
            ...
            else
            {
                ...
                using (GLFWriter writer = new GLFWriter(stream, m_SessionSummary, majorVersion, minorVersion))
                {
                    ...
                    if (m_Analysis != null)
                    {
                        //Analysis is designed to do all its own persistence management, much like session itself, so we just
                        //want to invoke that persistence
                        //BUG:  Need to figure out how to persist an analysis with new serialization model.
                        //writer.Write(m_Analysis);
                    }
