    ...
    internal class LogMessageEnumerator : IEnumerator<LogMessage>
    {
        ...
        public bool MoveNext()
        {
            ...
            while (m_PacketManager != null)
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        var stream = m_PacketManager.GetNextPacket();
                        if (stream == null)
                            break;
                    ...
                    catch (Exception ex)
                    {
                        ...
                        var zlibException = ex as ZlibException;
                        if (((serializationException != null) && serializationException.StreamFailed)
                            || (zlibException != null))
                        {
                            if (!Log.SilentMode)
                                Log.Write(LogMessageSeverity.Error, LogWriteMode.Queued, ex, Session.LogCategory,
                                          "Unable to continue deserializing data during packet stream read due to " + ex.GetBaseException().GetType(),
                            ...
                            break; //move to the next stream, this one is busted.
                        }
                        else
                        {
                            if (!Log.SilentMode)
                                Log.Write(LogMessageSeverity.Error, LogWriteMode.Queued, ex, Session.LogCategory,
                                    "Discarding packet and continuing due to " + ex.GetBaseException().GetType(),
        ...
        private void MoveNextReader()
        {
            ...
            if (m_UnloadedReaders.Count > 0)
            {
                ...
                {
                    if (!Log.SilentMode)
                        Log.Write(LogMessageSeverity.Information, LogWriteMode.Queued, ex, Session.LogCategory, "Aborting loading packet stream.", "A thread abort exception was thrown on our worker thread so we'll stop trying to load the session.");
