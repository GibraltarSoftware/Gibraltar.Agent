...
namespace Gibraltar.Monitor.Internal
{
    ...
    internal class SessionSummaryCollection : ISessionSummaryCollection
    {
        ...
        private bool RemoveItem(ISessionSummary item)
        {
            ...
            //we want to be sure we remove any item with this key, not just this exact object to preserve collection symmetry.
            if (m_Dictionary.Remove(item.Id))
            {
                m_List.Remove(item);
                return true;
            }
        ...
        public void RemoveAt(int index)
        {
            lock(m_Lock)
            {
                //find what the hell item they're talking about and remove that using our other overload.
