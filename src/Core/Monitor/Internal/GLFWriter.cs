...
namespace Gibraltar.Monitor.Internal
{
    internal class GLFWriter : IDisposable
    {
...
#if !DEBUG
            // We only want this behavior for release builds, for our own debug builds
            // we might prefer the writing to be the more typical optimized behavior.
            Synchronous = Debugger.IsAttached;
#endif
        }
        ...
        public bool AutoFlush { get { return m_AutoFlush; } set { m_AutoFlush = value; } }
        ...
        public void Flush()
        {
            ...
            if (count > 0)
            {
                ...
                {
                    byte[] array = ((MemoryStream) m_PacketStream).GetBuffer();
        ...
        /// <summary>
        /// When true, change the FlushMode on the GZipStream to FlushMode.Sync
        /// to be more aggressive about writing data to disk on flush.
        /// </summary>
        public bool Synchronous
        {
            get
            {
                var gzipStream = m_PacketStream as GZipStream;
                if (gzipStream == null )
                    return false;

                return gzipStream.FlushMode == FlushType.Sync;
            }
            ...
            set
            {
                ...
                {
                    gzipStream.FlushMode = FlushType.None;
                }
            }
        }
        ...
        public virtual void Close(bool isLastFile)
        {
            ...
            //we don't own the file stream, so we shouldn't close or dispose it.  We do own our buffer, so handle that.
            m_PacketStream.Close();
