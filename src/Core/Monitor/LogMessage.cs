...
namespace Gibraltar.Monitor
{
    ...
    public class LogMessage : IDisplayable, IDataObject, IComparable<LogMessage>, IEquatable<LogMessage>
    {
        ...
        private DataExtension m_Extension;  //used to add analysis properties, etc. 
        ...
        /// <summary>
        /// The current data extension object.  Owned by the session analysis.
        /// </summary>
        public DataExtension Extension
        {
            get
            {
                //if we don't have an extension yet, we'll have to create one.
                if (m_Extension == null)
                {
                    //see if it's in our session's analysis but we just haven't linked up with it.
                    m_Session.Analysis.DataExtensions.TryGetValue(((IDataObject)this).Id, out m_Extension);

                    if (m_Extension == null)
                    {
                        //it doesn't exist yet in our session analysis - add it.
                        m_Extension = m_Session.Analysis.DataExtensions.Add(this);
                    }
                }
