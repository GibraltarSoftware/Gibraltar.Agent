...
namespace Gibraltar.Monitor
{
    ...
    public sealed class ThreadInfoCollection : IList<ThreadInfo>
    {
        ...
        public bool Remove(ThreadInfo item)
        {
            ...
            lock (m_Lock)
            {
                if (m_ThreadInfoByGuid.Remove(item.Id))
                {
                    // We don't need to re-sort after a removal (unless already needed).
        ...
        public void UniquifyThreadNames()
        {
            lock (m_Lock)
            {
                ...
                Dictionary<string, ThreadInfo> currentInstanceByName = new Dictionary<string, ThreadInfo>(StringComparer.OrdinalIgnoreCase);
