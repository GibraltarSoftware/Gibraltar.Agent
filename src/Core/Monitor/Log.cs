...
namespace Gibraltar.Monitor
{
    ...
    public static class Log
    {
        ...
        //The cache of thread objects we need for creating log messages quickly
        //private static readonly Dictionary<int, ThreadInfo> g_ThreadInfoCollection = new Dictionary<int, ThreadInfo>();
        ...
        private static AutoSendConsent s_AutoSendConsent; //PROTECTED BY CONSENT LOCK
        ...
        /// <summary>
        /// Display the live viewer if available, foregrounding it if it's already displayed
        /// </summary>
        /// <remarks>This basically does the same thing has pressing the hot key</remarks>
        public static void ShowLiveViewer()
        {
            if (s_Initialized == false)
                return;

            //we need to get the CLR Listener
            Listener.ShowViewerForm();
        }

        /// <summary>
        /// Resets the current live viewer buffer, clearing the messages
        /// </summary>
        public static void ClearLiveViewer()
        {
            if (s_Initialized == false)
                return;

            Listener.ClearViewerForm();
        }
        ...
        public static bool AutoSendConsentGranted
        {
            get
            {
                if (s_Initialized == false)
                ...
                //first, do we even need consent?
                if (Configuration.AutoSendConsent.Enabled == false)
                {
                    //no, we don't.
                    return true;
                }
                ...
                lock (s_ConsentLock)
                {
                    if (s_AutoSendConsent == null)
                    {
                        //we need to go and try to get the current consent
                        try
                        {
                            string productName;
                            string applicationName;
                            GetConsentScope(out productName, out applicationName);

                            AutoSendConsent bestConsent = Repository.GetAutoSendConsent(SessionSummary.Product, applicationName);

                            s_AutoSendConsent = bestConsent;
                        }
                        catch (Exception ex)
                        {
                            GC.KeepAlive(ex);
                    ...
                    if (s_AutoSendConsent == null)
                    {
                        consentGranted = Configuration.AutoSendConsent.ConsentDefault;
                    }
                    else
                    {
                        consentGranted = s_AutoSendConsent.OptIn;
                    }

                    System.Threading.Monitor.PulseAll(s_ConsentLock);
                }
        ...
        /// </summary>
        /// <remarks>Auto send doesn't enable immediately if auto send consent is enabled to allow 
        /// time for the UI to be displayed and a user to make their choice.  In applications that can display
        /// a user interface DisplayStartupConsentDialog should be called to give the user the opportunity to 
        /// opt in and then enable auto send based on their decision.  For other applications (like
        /// windows services, console applications, and web sites) this method should be called to have
        /// the system evaluate the current auto-send configuration and if necessary start.</remarks>
        public static void ApplyConsent()
        {
            string productName;
            string applicationName;
            GetConsentScope(out productName, out applicationName);
            AutoSendConsent bestConsent = Repository.GetAutoSendConsent(productName, applicationName);

            if (!SilentMode)
                Write(LogMessageSeverity.Verbose, AutoSendConsent.LogCategory, "Applying current Auto-send Consent", "Current Consent Details:\r\n{0}", bestConsent);

            //now get the best send consent we have for real to return.
            ActionApplyConsent(bestConsent);
        }

        /// <summary>
        /// WinForms clients should call this to display the auto send consent form on startup if necessary
        /// </summary>
        /// <returns>The current consent object which will be null if no decision has been made.</returns>
        /// <remarks>This method should be called by WinForms applications at a convenient point near the start
        /// of the application to allow the agent to prompt for startup consent if necessary.  Prompting will
        /// only occur if no decision has been made yet and we haven't exceeded the prompting limit.</remarks>
        public static void DisplayStartupConsentDialog()
        {
            ...
            if (!SilentMode)
                Write(LogMessageSeverity.Verbose, AutoSendConsent.LogCategory, "Display Startup Consent Dialog Requested", null);

            if (s_StartupConsentDisplayed)
                return; //we want to be sure we only prompt once per launch.

            //find out if we have already made a decision.
            string productName;
            string applicationName;
            GetConsentScope(out productName, out applicationName);

            AutoSendConsent bestConsent = Repository.GetAutoSendConsent(SessionSummary.Product, applicationName);
            AutoSendConsentConfiguration config = Configuration.AutoSendConsent;

            if ((AutoSendConsent.IsNullOrNoSelection(bestConsent)) && (config.PromptUserOnStartupLimit > 0))
            {
                //they haven't made a decision - but can we still prompt them?
                if ((bestConsent == null)
                    || (bestConsent.UserPrompts < config.PromptUserOnStartupLimit)
                    || (bestConsent.ApplicationVersion != SessionSummary.ApplicationVersion))
                {
                    if (!SilentMode)
                        Write(LogMessageSeverity.Information, AutoSendConsent.LogCategory, "User will be prompted to make a consent decision", "There is no consent decision on file and we are still allowed to prompt the user.\r\nConsent Detail: {0}\r\n", bestConsent);

                    //yes, so now we really need to have a consent.
                    if (bestConsent == null)
                    {
                        //we will need to create a new one
                        bestConsent = Repository.AddOrGetAutoSendConsent(productName, applicationName, SessionSummary.ApplicationVersion);
                    }
                    else if (bestConsent.ApplicationVersion != SessionSummary.ApplicationVersion)
                    {
                        //we tried before but it was for a different version - doesn't count.
                        bestConsent.UserPrompts = 0;
                        if (!SilentMode)
                            Write(LogMessageSeverity.Information, AutoSendConsent.LogCategory, "Resetting startup prompt count because application version has changed", "The user previously never made a decision under a different version so we'll start counting again.\r\nConsent Detail: {0}\r\n", bestConsent);
                    }

                    DialogResult result = ActionEditConsent(bestConsent);
                    if (result == DialogResult.Cancel)
                    {
                        //increment our startup prompts since we did prompt them.
                        bestConsent.UserPrompts += 1;
                        bestConsent.SelectionMade = false;
                        bestConsent.OptIn = false;
                    }

                    //save the consent result.
                    Repository.SaveAutoSendConsent(bestConsent);

                    //now get the best send consent we have for real to return.
                    bestConsent = Repository.GetAutoSendConsent(productName, applicationName);
                }
            }

            //Now apply the consent - even if we didn't display we have to do this or we won't start
            //handling background send requests, etc.
            ActionApplyConsent(bestConsent);

            if (!SilentMode)
                Write(LogMessageSeverity.Verbose, AutoSendConsent.LogCategory, "Best consent determined", "Consent Details:\r\n{0}", bestConsent);

            s_StartupConsentDisplayed = true; //whether we did or not, they selected or not - it was attempted.
        }


        /// <summary>
        /// WinForms clients should call this to display the auto send consent form outside of the startup sequence.
        /// </summary>
        /// <returns>The current consent object which will be null if no decision has been made.</returns>
        /// <remarks>The form will always display if auto send consent is configured even if a selection has 
        /// already been made.</remarks>
        public static void DisplayConsentDialog()
        {
            if (s_Initialized == false)
                return;
            ...
            string applicationName;
            GetConsentScope(out productName, out applicationName);
            AutoSendConsent bestConsent = Repository.AddOrGetAutoSendConsent(productName, applicationName, SessionSummary.ApplicationVersion);

            if (!SilentMode)
                Write(LogMessageSeverity.Verbose, AutoSendConsent.LogCategory, "Display Consent Dialog Requested", "Current Consent Details:\r\n{0}", bestConsent);
            ...
            if (result == DialogResult.OK)
            {
                ...
                Repository.SaveAutoSendConsent(bestConsent);
                if (!SilentMode)
                    Write(LogMessageSeverity.Information, AutoSendConsent.LogCategory, "User updated consent", "New Consent Details:\r\n{0}", bestConsent);
            }
        ...
        /// <summary>
        /// Explicitly trigger thread event subscriptions for the current thread.
        /// </summary>
        public static void RegisterUIThread()
        {
            EnsureSummaryIsAvailable();

            ApplicationType appType = s_SessionStartInfo.AgentAppType;
            if (appType == ApplicationType.Windows)
            {
                Listener.InitializeThreadListening(appType, true);
            }
        }
        ...
        /// <param name="severity">The log message severity.</param>
        /// <param name="category">The category for this log message.</param>
        /// <param name="detailsXml">Optional.  Serialized data to record with the message</param>
        /// <param name="caption">A simple single-line message caption. (Will not be processed for formatting.)</param>
        /// <param name="description">Additional multi-line descriptive message (or may be null) which can be a format string followed by corresponding args.</param>
        /// <param name="args">A variable number of arguments referenced by the formatted description string (or no arguments to skip formatting).</param>
        public static void WriteDetail(LogMessageSeverity severity, string category, string detailsXml, string caption, string description, params object[] args)
        {
            if (s_Initialized == false)
                return;

            // skipFrames = 1 to skip out to our caller instead of right here.
            LocalLogMessage logMessage = new LocalLogMessage(severity, ThisLogSystem, category, 1, detailsXml, caption,
                description, args);
            logMessage.PublishToLog(); // tell the SimpleLogMessage to publish itself (back through us).
        }
        ...
        /// <summary>
        /// Record an unexpected Exception to the Gibraltar central log, formatted automatically, and report it to the user.
        /// </summary>
        /// <remarks><para>This method provides an easy way to record an Exception as a separate message which will be
        /// attributed to the code location which threw the Exception rather than where this method was called from.
        /// The category will default to "Exception" if null, and the message will be formatted automatically based on the
        /// Exception.  The severity will be determined by the canContinue parameter:  Critical for fatal errors (canContinue
        /// is false), Error for non-fatal errors (canContinue is true).</para>
        /// <para>This method is intended for use with top-level exception catching for errors not anticipated in a specific
        /// operation, when the user should be alerted because the error impacts their work flow.  For unanticipated errors
        /// which do not disrupt a user activity, see the <see CREF="RecordException">RecordException</see> method.</para>
        /// <para>Only Windows applications can report to the user.  Services, console applications, and web applications
        /// will only record the exception (as with the RecordException method).</para></remarks>
        /// <param name="skipFrames">The number of stack frames to skip back over to determine the original caller.</param>
        /// <param name="exception">An Exception object to record as a log message.  This call is ignored if null.</param>
        /// <param name="detailsXml">Optional.  An XML document with extended details about the exception.  Can be null.</param>
        /// <param name="category">The application subsystem or logging category that the message will be associated with.</param>
        ...
        /// <returns><see CREF="DialogResult.None">None</see> if not waiting for response (or could not),
        /// <see CREF="DialogResult.No">No</see> if waiting ended without a specific user selection,</returns>
        [MethodImplAttribute(MethodImplOptions.NoInlining)]
        public static DialogResult ReportException(int skipFrames, Exception exception, string detailsXml, string category,
                                                   bool canContinue, bool waitForResponse)
        {
            if (s_Initialized == false)
            ...
            return ReportException(sourceProvider, exception, detailsXml, category, canContinue, waitForResponse);
        }
        ...
        /// <returns><see CREF="DialogResult.None">None</see> if not waiting for response (or could not),
        /// <see CREF="DialogResult.No">No</see> if waiting ended without a specific user selection,</returns>
        public static DialogResult ReportException(IMessageSourceProvider sourceProvider, Exception exception, string detailsXml,
                                                   string category, bool canContinue, bool waitForResponse)
        {
            ...
            bool reporting = (s_SessionStartInfo.AgentAppType == ApplicationType.Windows);

            if (IsMonoRuntime)
            {
                RecordException(sourceProvider, exception, detailsXml, category, canContinue, false, false);

                Write(LogMessageSeverity.Information, "Gibraltar.Agent.Error Manager", "Gibraltar Error Manager is not supported on Mono",
                      "The Gibraltar Error Manager is designed to run in a separate UI thread, but the Mono runtime does not " +
                      "support multiple UI threads as of version 2.6, so the GEM is disabled when running under Mono with this "+
                      "version of the Agent.\r\n");
            }
            else
            {
                RecordException(sourceProvider, exception, detailsXml, category, canContinue, reporting, waitForResponse);
            }

            if (reporting) // Mono will automatically suppress and return immediately.
            {
                ExceptionDisplayRequest request = new ExceptionDisplayRequest(exception, waitForResponse, canContinue);
                Listener.ShowAlertDialog(request);
                if (waitForResponse)
                {
        ...
        /// <remarks>This method is not safe from within the log initialize event.</remarks>
        public static string CalculateWebUrl(string pageFileName)
        {
            EnsureSummaryIsAvailable();

            SessionSummary summary = SessionSummary;
            string fullUrl = CommonCentralLogic.CalculateWebUrl(pageFileName, summary.Product, summary.Application, summary.ApplicationVersion);
        ...
        /// </summary>
        /// <param name="pageFileName">The simple page file name (including ".aspx" extension).</param>
        /// <remarks>This method is not safe from within the log initialize event.  Call CommonFileTools.DisplayWebPage
        /// directly if a SessionSummary object is not available.</remarks>
        public static void DisplayWebPage(string pageFileName)
        {
            EnsureSummaryIsAvailable();

            SessionSummary summary = SessionSummary;
            CommonCentralLogic.DisplayWebPage(pageFileName, summary.Product, summary.Application, summary.ApplicationVersion);
        }
        ...
        public static bool SendSessions(SessionCriteria? criteria, Predicate<ISessionSummary> sessionMatchPredicate, bool asyncSend)
        {
            ...
            else
            {
                ...
                {
                    string message = null;
                    if (IsHubSubmissionConfigured(ref message) && Packager.CanSendToServer(out message))
                    {
                        ...
                        if (criteria.HasValue)
                            newPackager.SendToServer(criteria.Value, true, config.PurgeSentSessions, false, null, false, null, null, 0, false, null, null, asyncSend);
                        else
                            newPackager.SendToServer(sessionMatchPredicate, true, config.PurgeSentSessions, false, null, false, null, null, 0, false, null, null, asyncSend);

                        result = true;
                    }
                    else if (IsEmailSubmissionConfigured(ref message))
                    {
                        //we DON'T release the active packager here, we do it in the event handler.
                        newPackager.EndSend += Packager_EndSend;
                        if (criteria.HasValue)
                            newPackager.SendEmail(criteria.Value, true, null, null, null, null, 0, null, null, null, asyncSend);
                        else
                            newPackager.SendEmail(sessionMatchPredicate, true, null, null, null, null, 0, null, null, null, asyncSend);
                                                       ...
                                                       string detailsXml, string caption, string description, params object[] args)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(userName))
                {
                    try
                    {
                        ...
                        IIdentity threadIdentity = (userPrincipal != null) ? userPrincipal.Identity : null;
            ...
            else
            {
                ...
                formattedDescription = ErrorNotifier.SafeFormat(CultureInfo.CurrentCulture, description, args);
            }
        ...
        private static ThreadInfo GetCurrentThreadInfo()
        {
            ...
            if (t_CurrentThreadInfo == null)
            {
                ...
                // Let the listener know about this thread?
                // Don't bother to start listening on this thread if we're exiting.
                // Only register on STA threads, not MTA or unknown.
                if (IsSessionEnding == false && s_SessionStartInfo.AgentAppType == ApplicationType.Windows &&
                    Thread.CurrentThread.GetApartmentState() == ApartmentState.STA)
                {
                    Listener.InitializeThreadListening(s_SessionStartInfo.AgentAppType, false);
                }
            }
        ...
        private static DialogResult ActionEditConsent(AutoSendConsent consent)
        {
            DialogResult result = DialogResult.Cancel;

            //make sure we have enough configuration to display the dialog.
            AutoSendConsentConfiguration consentConfig = Configuration.AutoSendConsent;
            if (consentConfig.Enabled
                && (string.IsNullOrEmpty(consentConfig.CompanyName) == false)
                && (string.IsNullOrEmpty(consentConfig.ServiceName) == false))
            {
                using (AutoSendConsentDialog consentDialog = new AutoSendConsentDialog())
                {
                    result = consentDialog.UpdateConsent(consent, consentConfig.PrivacyPolicyUrl, consentConfig.ServiceName, consentConfig.CompanyName);
                }
            }
        ...
        /// <summary>
        /// Handles runtime changes to the consent configuration.
        /// </summary>
        /// <param name="newConstent"></param>
        private static void ActionApplyConsent(AutoSendConsent newConstent)
        {
            if (newConstent == null)
            {
#if DEBUG
                DebugBreak();
#endif
                return; //should never happen, but we generally don't want to throw exceptions.
            }

            lock (s_ConsentLock)
            {
                //update our consent object on file
                s_AutoSendConsent = newConstent;

                //and trigger the right response from the publish engine.
                if (newConstent.OptIn)
                {
                    StartPublishEngine(); //this checks to see if it really should be enabled based on the current configuration
                }
                else
                {
                    ShutdownPublishEngine();
                }
        ...
        private static string GetPackagerFileNamePath()
        {
            ...
            string proposedPath = Assembly.GetExecutingAssembly().CodeBase;
        ...
        private static void SendSessionData()
        {
            try
            {
                ...
                packagerStartInfo.ErrorDialog = false;
                ...
                else
                {
                    ...
                    //now see if we should be overriding the server information.  
                    EmailConfiguration email = s_RunningConfiguration.Email;
                    if (string.IsNullOrEmpty(email.Server) == false)
                    {
                        //yep, so lets see WHAT we should be overriding.
                        argumentString.AppendFormat("/server \"{0}\" ", email.Server);

                        if (string.IsNullOrEmpty(email.User) == false)
                        {
                            argumentString.AppendFormat("/user \"{0}\" /password \"{1}\" ", email.User, email.Password);
                        }
                        ...
                        if (email.Port != 0)
                        {
                            argumentString.AppendFormat("/port \"{0}\" ", email.Port);
                        }
                    }
                }
        ...
        private static bool OnInitialize(AgentConfiguration configuration, bool suppressTraceInitialize)
        {
            ...
            if (s_RunningConfiguration.AutoSendConsent.Enabled == false) //make sure we don't have to wait for consent information to load.
