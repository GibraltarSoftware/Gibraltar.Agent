    ...
    public class LocalRepository : IRepository
    {
        ...
        public bool Remove(Guid sessionId)
        {
            lock (m_Lock)
            {
                ...
                {
                    fileRemoved = Win32Helper.SafeDeleteFile(fragment.FullName) || fileRemoved;
                }
                return fileRemoved;
            }
        }
        ...
        public bool Remove(Guid sessionId, Guid fileId)
        {
            lock (m_Lock)
            {
                ...
                return Win32Helper.SafeDeleteFile(victim.FullName);
            }
        }
        ...
        public static SessionHeader LoadSessionHeader(string sessionFileNamePath)
        {
            ...
            try
            {
                using (var sourceFile = Win32Helper.OpenFileStream(sessionFileNamePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    if (sourceFile == null)
                    {
#if DEBUG
                ...
                using (InterprocessLock sessionLock = GetRunningSessionLock(session.Id))
                {
                    ...
                    foreach (var fileFragment in session.Fragments)
                    {
                        ...
                        try
                        {
                            ...
                            sourceFile = Win32Helper.OpenFileStream(fileFragment.FullName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);
                            if (sourceFile == null)
                            {
        ...
        private bool SetSessionNew(string destinationDirectory, Guid sessionId, bool isNew)
        {
            ...
            if (m_SessionCache.TryGetValue(sessionId, out sessionFileInfo))
            {
                foreach (var fragment in sessionFileInfo.Fragments)
                {
                    try
                    {
                        if (!destinationDirectory.Equals(fragment.DirectoryName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            ...
                            Win32Helper.SafeDeleteFile(destinationFileNamePath);
