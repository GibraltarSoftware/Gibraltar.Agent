...
namespace Gibraltar.Data
{
    ...
    public sealed class SessionHeader: ISessionSummary, IEquatable<SessionHeader>
    {
        ...
        private OSVersionInfo m_VersionInfo;
            ...
            :this(sessionSummary.Properties)
        {
            ...
            MessageCount = (int)sessionSummary.MessageCount;
            CriticalCount = (int)sessionSummary.CriticalCount;
            ErrorCount = (int)sessionSummary.ErrorCount;
            WarningCount = (int)sessionSummary.WarningCount;
        ...
        public string OSFamilyName
        {
            ...
            {
                EnsureOSInfoLoaded();
                return m_VersionInfo.FamilyName;
            }
        }
        ...
        public string OSEditionName
        {
            ...
            {
                EnsureOSInfoLoaded();
                return m_VersionInfo.EditionName;
            }
        }
        ...
        public string OSFullNameWithServicePack
        {
            ...
            {
                EnsureOSInfoLoaded();
                return m_VersionInfo.FullNameWithServicePack;
            }
        }
        ...
        /// </summary>
        /// <remarks>The session summary can be transferred separately from the session details
        ...
        private bool LoadStream(Stream rawData, int length)
        {
            ...
            if (!string.IsNullOrEmpty(applicationVersionRaw)) //be as tolerant as possible on read
            ...
            if (!string.IsNullOrEmpty(agentVersionRaw)) //be as tolerant as possible on read
            ...
            if (!string.IsNullOrEmpty(rawOSVersion)) //be as tolerant as possible on read
            ...
            if (!string.IsNullOrEmpty(osArchitectureRaw)) //be as tolerant as possible on read
            ...
            if (!string.IsNullOrEmpty(osBootModeRaw)) //be as tolerant as possible on read
                m_OSBootMode = (BootMode)Enum.Parse(typeof(BootMode), osBootModeRaw, true);
            ...
            if (!string.IsNullOrEmpty(rawRuntimeVersion)) //be as tolerant as possible on read
            ...
            if (!string.IsNullOrEmpty(runtimeArchitectureRaw)) //be as tolerant as possible on read
        ...
        private void EnsureOSInfoLoaded()
        {
            lock(m_Lock)
            {
                 m_VersionInfo = new OSVersionInfo(OSPlatformCode, OSVersion, OSServicePack, OSSuiteMask, OSProductType);                
            }
        }
