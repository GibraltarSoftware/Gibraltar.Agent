    ...
    internal class RepositoryPublishEngine
    {
        ...
        private void CreateMessageDispatchThread()
        {
            lock (m_SessionPublishThreadLock)
            {
                ...
                m_SessionPublishThread.TrySetApartmentState(ApartmentState.MTA);
                m_SessionPublishThread.IsBackground = true;
                m_SessionPublishThread.Priority = ThreadPriority.BelowNormal;
        ...
        private void RepositoryPublishMain()
        {
            ...
            try
            {
                //see if we are even allowed to publish before proceeding.  This check does invoke the repository so
                //we never want to make it on a user thread (thus why we're doing it here)
                if (Log.AutoSendConsentGranted == false)
                {
                    //the user has not consented to publishing, we want to exit quickly but still go through our shutdown logic.
                    if (!Log.SilentMode) Log.Write(LogMessageSeverity.Information, LogCategory, "User has not consented to automatically send sessions", "The repository publish engine will shut down because the user has not consented to automatically send sessions.");
                }
                else
                {
        ...
        private void RepositoryPublishLoop()
        {
            ...
            if (m_Initialized)
            {
                while (m_StopRequested == false)
                {
                    ...
                    if (m_Client.CanConnect(out status, out statusMessage))
                    {
                        ...
                        m_Client.PublishSessions(false, m_Configuration.Server.PurgeSentSessions);
                    }
                    else
                    {
                        if (status == HubStatus.Expired)
                        {
                        ...
                        Log.Write(LogMessageSeverity.Warning, LogCategory, "Sessions will not be published to the server because it can't be contacted",
                                  "Status: {0}\r\nMessage: {1}\r\nCheck Interval: {2}", status, statusMessage, m_CheckInterval);
#endif
                    }
