...
namespace Gibraltar.Data
{
    ...
    public class AutoSendConsent
    {
        ...
        public static AutoSendConsent LoadFile(string fileNamePath)
        {
            ...
            using(FileStream stream = Win32Helper.OpenFileStream(fileNamePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                if (stream != null)
                {
                    try
        ...
        public void Save(string fileNamePath)
        {
            ...
            {
                using (var stream = Win32Helper.OpenFileStream(fileNamePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None))
                {
                    if (stream == null)
                    {
                        fileAttempts++;
