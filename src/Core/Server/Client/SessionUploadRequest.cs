    ...
    internal class SessionUploadRequest : WebChannelRequestBase, IDisposable
    {
        ...
        private void PerformCleanup(IWebChannelConnection connection)
        {
            try
            {
                ...
                connection.UploadData(GenerateResourceUri(), "DELETE", BinaryContentType, new byte[] { });
            }
            catch (Exception ex)
            {
                GC.KeepAlive(ex);
        ...
        protected override void OnProcessRequest(IWebChannelConnection connection)
        {
            ...
            using (var sessionStream = Repository.LoadSessionFileStream(SessionId, FileId.Value))
            {
                ...
                if (sessionStream.Length < SinglePassCutoffBytes)
                {
                    ...
                    connection.UploadData(GenerateResourceUri(), "PUT", BinaryContentType, sessionData, additionalHeaders);
                }
                else
                {
                    ...
                    while (m_BytesWritten < sessionStream.Length)
                    {
                        ...
                        try
                        {
                            connection.UploadData(requestUrl, "POST", BinaryContentType, sessionData, additionalHeaders);
                        }
        ...
        {
            return FileId.HasValue ?  string.Format("/Hosts/{0}/Sessions/{1}/Files/{2}.zip", ClientId, SessionId, FileId) :
                string.Format("/Hosts/{0}/Sessions/{1}/session.glf", ClientId, SessionId);
        }
