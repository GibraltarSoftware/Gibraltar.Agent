...
namespace Gibraltar.Agent.Metrics
{
    ...
    public sealed class SampledMetricDefinition : IMetricDefinition, IComparable<SampledMetricDefinition>, IEquatable<SampledMetricDefinition>
    {
        ...
        internal static SampledMetricDefinition[] RegisterAll(object userInstanceObject)
        {
            ...
            if (userInstanceObject != null)
            {
                ...
                else
                {
                    ...
                    Type baseObjectType = userObjectType.BaseType;

                    // The IsInterface check shouldn't be needed, but just in case, we want to be sure we don't cause a duplicate.
                    while (baseObjectType != null && baseObjectType != typeof (object) && baseObjectType.IsInterface == false)
                    {
                        ...
                        baseObjectType = baseObjectType.BaseType; // Get the next deeper Type.
                    }
        ...
        internal static SampledMetricDefinition[] RegisterType(Type userObjectType)
        {
            ...
            {
                throw new ArgumentNullException("userObjectType", "A valid Type must be provided.");
            }
        ...
        internal static SampledMetricDefinition Register(Type userObjectType, string counterName)
        {
            ...
            {
                throw new ArgumentNullException("counterName", "A valid counter name must be specified to select a single sampled metric definition, or use the overload without a counterName parameter.");
            }
        ...
        */
        ...
        internal static SampledMetricDefinition[] RegisterGroup(Type userObjectType, string counterName)
        {
            ...
            lock (s_DictionaryLock)
            {
                if (s_DefinitionsMap.TryGetValue(userObjectType, out definitions) == false || definitions == null)
                {
                    ...
                    Attribute rawSampledMetricAttribute = Attribute.GetCustomAttribute(userObjectType, typeof (SampledMetricAttribute));
                    // Try to cast it to the specific kind of attribute we need
                    SampledMetricAttribute sampledMetricAttribute = rawSampledMetricAttribute as SampledMetricAttribute;
                    ...
                    MemberInfo[] members = userObjectType.FindMembers(MemberTypes.Field | MemberTypes.Method | MemberTypes.Property,
                    ...
                    foreach (MemberInfo curMember in members)
                    {
                        ...
                        if (curMember.IsDefined(typeof (SampledMetricInstanceNameAttribute), false))
                        {
                            ...
                            if (instanceNameBinding != null)
                            {
                                ...
                                Trace.TraceWarning(
                                    "There is more than one member on the type {0} with the SampledMetricName attribute.  The member {1} will be used because it was found first.",
                                    userObjectType.FullName, instanceNameBinding.Name);
                            }
                            else
                            {
                        ...
                        if (curMember.IsDefined(typeof (SampledMetricDivisorAttribute), false))
                        {
                            ...
                            object[] curMemberValueAttributes = curMember.GetCustomAttributes(typeof (SampledMetricDivisorAttribute), false);

                            foreach (object curMemberValueAttribute in curMemberValueAttributes)
                            {
                                //cast it and test
                                SampledMetricDivisorAttribute curDivisorAttribute =
                                    curMemberValueAttribute as SampledMetricDivisorAttribute;
                                if (curDivisorAttribute != null)
                                {
                                    ...
                                    {
                                        Trace.TraceWarning("Sampled metric attribute usage warning:\r\n" +
                                                           "SampledMetricDivisor attribute on {0} member {1} has an invalid counter name ({2}) and will have no effect.\r\n",
                                                           userObjectType, curMember.Name, ((divisorCounterName == null) ? "null" : "\"\""));
                                    }
                                                                               ...
                                                                               out existingMetricDefinition))
                                    {
                                        ...
                                        if (sampledMetricDefinition == null)
                                        {
                                            ...
                                            Trace.TraceWarning("Sampled metric attribute usage warning:\r\n" +
                                                               "Unable to add {0} member {1} as divisor for sampled metric counter {2}:\r\n" +
                                                               "A metric definition already exists with the same namespace ({3}), category ({4}),\r\n" +
                                                               "and counter name ({2}), but it is not a sampled metric!\r\n" +
                                                               "Make sure each metric definition uses a unique name.\r\n",
                                                               userObjectType, curMember.Name, divisorCounterName, metricsSystem,
                                                               metricCategoryName);
                                        }
                                        else if (sampledMetricDefinition.IsBound == false || sampledMetricDefinition.BoundType != userObjectType)
                                        {
                                            ...
                                            Trace.TraceWarning("Sampled metric attribute usage warning:\r\n" +
                                                               "Unable to add {0} member {1} as divisor for sampled metric counter {2}:\r\n" +
                                                               "A metric definition already exists with the same namespace ({3}), category ({4}),\r\n" +
                                                               "and counter name ({2}), but is was not defined from this type ({0}).\r\n" +
                                                               "Make sure each metric definition uses a unique name.\r\n",
                                                               userObjectType, curMember.Name, divisorCounterName, metricsSystem,
                                                               metricCategoryName);
                                        }
                                        else if (BindingMatchesMember(sampledMetricDefinition.DivisorBinding, curMember) == false)
                                        {
                                            ...
                                            Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                               "Unable to add {0} member {1} as divisor for sampled metric counter {2}:\r\n" +
                                                               "A metric definition already exists with the same namespace ({3}), category ({4}),\r\n" +
                                                               "and counter name ({2}), but its divisor was not bound to this member ({1}).\r\n" +
                                                               "Make sure each metric definition uses a unique name.\r\n",
                                                               userObjectType, curMember.Name, divisorCounterName, metricsSystem,
                                                               metricCategoryName);
                                        }
                                    ...
                                    else if (divisors.ContainsKey(divisorCounterName))
                                    {
                                        ...
                                        Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                           "SampledMetricDivisor attribute on {0} member {1} specifies a counter name ({2})\r\n" +
                                                           "which has already been specified on member {3}.  Only the first occurrence will be used.\r\n",
                                                           userObjectType, curMember.Name, divisorCounterName,
                                                           divisors[divisorCounterName].Name);
                                    }
                                    else
                                    {
                                        divisors[divisorCounterName] = curMember; // Remember this member for later.
                    ...
                    foreach (MemberInfo curMember in members)
                    {
                        ...
                        if (curMember.IsDefined(typeof (SampledMetricValueAttribute), false))
                        {
                            ...
                            //we get back an array of objects just in case there are any non-CLS compliant attributes defined, which there never are.
                            object[] curMemberValueAttributes = curMember.GetCustomAttributes(typeof (SampledMetricValueAttribute), false);

                            foreach (object curMemberValueAttribute in curMemberValueAttributes)
                            {
                                //cast it and test
                                SampledMetricValueAttribute curValueAttribute = curMemberValueAttribute as SampledMetricValueAttribute;
                                    ...
                                    lock (metricDefinitionsLock)
                                    {
                                                                              ...
                                                                              out rawMetricDefinition))
                                        {
                                            ...
                                            if (sampledMetricDefinition == null)
                                            {
                                                // Uh-oh, the definition already exists, but it isn't a sampled metric!
                                                Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                                   "Unable to add {0} member {1} as sampled metric counter {2}:\r\n" +
                                                                   "A metric definition already exists with the same namespace ({3}), category ({4}),\r\n" +
                                                                   "and counter name ({2}), but is not a sampled metric.\r\n" +
                                                                   "Make sure each metric definition uses a unique name.\r\n",
                                                                   userObjectType, curMember.Name, metricCounterName, metricsSystem,
                                                                   metricCategoryName);
                                            }
                                            ...
                                            else if (BindingMatchesMember(sampledMetricDefinition.DataBinding, curMember) == false)
                                            {
                                                ...
                                                Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                                   "Unable to add {0} member {1} as sampled metric counter {2}:\r\n" +
                                                                   "A metric definition already exists with the same namespace ({3}), category ({4}),\r\n" +
                                                                   "and counter name ({2}), but is was not bound to this member ({1}).\r\n" +
                                                                   "Make sure each metric definition uses a unique name.\r\n",
                                                                   userObjectType, curMember.Name, metricCounterName, metricsSystem,
                                                                   metricCategoryName);
                                            }
                                        ...
                                        else if (curType == null) // Warn about an unreadable property.
                                        {
                                            Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                               "Unable to add {0} member {1} as sampled metric counter {2}:\r\n" +
                                                               "The property is has no get accessor and can not be read.\r\n",
                                                               userObjectType, curMember.Name, metricCounterName);
                                        }
                                        else if (curType == typeof (void)) // Warn about a void method.
                                        {
                                            Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                               "Unable to add {0} member {1} as sampled metric counter {2}:\r\n" +
                                                               "The method's return value is void.\r\n",
                                                               userObjectType, curMember.Name, metricCounterName);
                                        }
                                        else if (IsValidDataType(curType) == false)
                                        {
                                            Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                               "Unable to add {0} member {1} as sampled metric counter {2}:\r\n" +
                                                               "The member's Type ({3}) is not supported for sampled metrics.\r\n",
                                                               userObjectType, curMember.Name, metricCounterName, curType.Name);
                                        }
                                        else
                                        {
                                            ...
                                            if (divisors.TryGetValue(metricCounterName, out divisorInfo))
                                            {
                                                ...
                                                if (RequiresDivisor(samplingType) == false)
                                                {
                                                    ...
                                                    Trace.TraceWarning("Sampled metric attribute usage warning:\r\n" +
                                                                       "SampledMetricDivisor attribute on {0} member {4} specifies counter name ({2}),\r\n" +
                                                                       "but SampledMetricValue attribute on member {1} designates sampling type as {3} which does not use a divisor.\r\n",
                                                                       userObjectType, curMember.Name, metricCounterName, samplingType,
                                                                       divisorInfo.Name);
                                            ...
                                            else
                                            {
                                                ...
                                                if (RequiresDivisor(samplingType))
                                                {
                                                    ...
                                                    Trace.TraceWarning("Sampled metric attribute usage error:\r\n" +
                                                                       "SampledMetricValue attribute on {0} member {1} designates sampling type as {3} which requires a divisor,\r\n" +
                                                                       "but no corresponding SampledMetricDivisor attribute was found for its counter name ({2}).\r\n",
                                                                       userObjectType, curMember.Name, metricCounterName, samplingType);
                                                }
                                            }
        ...
        internal void WriteSample(string instanceName, object metricData)
        {
            if (metricData == null)
            {
                throw new ArgumentNullException("metricData");
            }
        ...
        internal BindingFlags GetBindingFlags(NameValuePair<MemberTypes> binding)
        {
            ...
            switch (binding.Value)
            {
                ...
                default:
                    ...
                    Trace.TraceWarning("Loupe data binding error:\r\n" +
                                       "Unrecognized member type (0x{0:X}: {0:F}) for {2} member name \"{1}\".\r\n" +
                                       "Using default metric instance for sampling: metric namespace ({3}), category ({4}), counter name ({5}).\r\n",
                                       binding.Value, binding.Name ?? "(null)", BoundType.Name,
                                       MetricsSystem, CategoryName, CounterName);
                    bindingFlags = 0;
                    break;
            }
        ...
        public static bool TryGetValue(Type userObjectType, string counterName, out SampledMetricDefinition value)
        {
            ...
            if (foundValue == false) // If we didn't find it on the known list for that type...
            {
                ...
                {
                    Attribute rawSampledMetricAttribute = Attribute.GetCustomAttribute(userObjectType, typeof(SampledMetricAttribute));
                    sampledMetricAttribute = rawSampledMetricAttribute as SampledMetricAttribute;
                }
