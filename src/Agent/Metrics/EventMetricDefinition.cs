...
namespace Gibraltar.Agent.Metrics
{
    ...
    public sealed class EventMetricDefinition : IMetricDefinition, IComparable<EventMetricDefinition>, IEquatable<EventMetricDefinition>
    {
        ...
        internal static EventMetricDefinition[] RegisterAll(object metricData)
        {
            ...
            if (metricData != null)
            {
                ...
                else
                {
                    ...
                    Type baseObjectType = (userObjectType.IsInterface) ? null : userObjectType.BaseType;
        ...
        internal static EventMetricDefinition RegisterType(Type metricDataObjectType)
        {
            ...
            lock (s_DictionaryLock)
            {
                if (s_DefinitionMap.TryGetValue(metricDataObjectType, out newMetricDefinition) == false)
                {
                    ...
                    Attribute rawEventMetricAttribute = Attribute.GetCustomAttribute(metricDataObjectType, typeof(EventMetricAttribute));
                    // Try to cast it to the specific kind of attribute we need
                    EventMetricAttribute eventMetricAttribute = rawEventMetricAttribute as EventMetricAttribute;
                    ...
                    lock (Log.MetricDefinitions.Lock)
                    {
                        ...
                        else
                        {
                            ...
                            MemberInfo[] members = metricDataObjectType.FindMembers(MemberTypes.Field | MemberTypes.Method | MemberTypes.Property,
                            ...
                            foreach (MemberInfo curMember in members)
                            {
                                ...
                                if (curMember.IsDefined(typeof(EventMetricInstanceNameAttribute), false))
                                {
                                    ...
                                    if (newMetricDefinition.NameBound)
                                    {
                                        ...
                                        Trace.TraceWarning(
                                            "There is more than one member on the type {0} with the EventMetricInstanceName attribute.  The member {1} will be used because it was found first.",
                                            metricDataObjectType.FullName, newMetricDefinition.NameMemberName);
                                    }
                                    else
                                    {
                                ...
                                if (curMember.IsDefined(typeof(EventMetricValueAttribute), false))
                                {
                                    ...
                                    object[] curMemberValueAttributes = curMember.GetCustomAttributes(typeof(EventMetricValueAttribute), false);

                                    foreach (object curMemberValueAttribute in curMemberValueAttributes)
                                    {
                                        //cast it and test
                                        EventMetricValueAttribute curValueAttribute = curMemberValueAttribute as EventMetricValueAttribute;
                                        if (curValueAttribute != null)
                                        {
                                            ...
                                            {
                                                Trace.TraceWarning(
                                                    "Unable to add member {1} as event value column {2} because event metric definition {0} already has another value with the same name.  Make sure each value has a unique name.",
                                                    newMetricDefinition.Key, curMember.Name, curValueAttribute.Name);
                                            }
                                            else if (curType == null) // Warn about an unreadable property.
                                            {
                                                Trace.TraceWarning(
                                                    "Unable to add property {1} to event metric definition {0} as event value column {2} because the property isn't readable.",
                                                    newMetricDefinition.Key, curMember.Name, curValueAttribute.Name);
                                            }
                                            else if (curType == typeof(void)) // Warn about a void method.
                                            {
                                                Trace.TraceWarning(
                                                    "Unable to add method {1} to event metric definition {0} as event value column {2} because its return value is void",
                                                    newMetricDefinition.Key, curMember.Name, curValueAttribute.Name);
                                            }
                                            else
                                            {
                                                //We finally have validated everything and we're ready to set up the new value.
                                                EventMetricValueDefinition newValue = newMetricDefinition.ValueCollection.Add(curValueAttribute.Name, curType);
                                                ...
                                                if ((newMetricDefinition.DefaultValue == null) && (curValueAttribute.IsDefaultValue))
                                                {
        ...
        public static void Register(ref EventMetricDefinition newDefinition)
        {
            ...
            {
                throw new ArgumentNullException("newDefinition", "A null definition can not be registered nor used to look up a registered event metric definition.");
            }
        ...
        public static void Register(ref EventMetricDefinition newDefinition, string defaultValue)
        {
            ...
            {
                throw new ArgumentNullException("defaultValue", "The specified defaultValue name must be a legal value column name and thus may not be null or empty.");
            }
            ...
            switch (binding.Value)
            {
                ...
                default:
                    ...
                    Trace.TraceWarning("Loupe data binding error:\r\n" +
                                       ...
                                       "Event metric definition:  metrics system ({3}), category name ({4}), counter name ({5}).\r\n",
        ...
        */
        ...
        public static bool TryGetValue(Type metricDataObjectType, out EventMetricDefinition value)
        {
            ...
            {
                value = null;
                throw new ArgumentNullException("metricDataObjectType");
            }
            ...
            if (foundValue == false || value == null)
            {
                ...
                {
                    Attribute rawEventMetricAttribute = Attribute.GetCustomAttribute(metricDataObjectType, typeof(EventMetricAttribute));
                    eventMetricAttribute = rawEventMetricAttribute as EventMetricAttribute;
                }
