    ...
    internal sealed class EventMetricValueDefinitionCollection : IList<EventMetricValueDefinition>
    {
        ...
        public EventMetricValueDefinition Add(string name, Type type, SummaryFunction summaryFunction, string unitCaption, string caption, string description)
        {
            lock (m_Definition.Lock)
            {
                ...
                if (ContainsKey(name))
                {
                    throw new ArgumentException("There is already a value definition with the provided name.", "name");
                }
        ...
        public void Add(EventMetricValueDefinition item)
        {
            ...
            {
                throw new ArgumentException("The provided value definition item is not associated with our metric definition", "item");
            }
            ...
            lock (m_Definition.Lock)
            {
                ...
                if (ContainsKey(item.Name))
                {
                    throw new ArgumentException(
                        string.Format(CultureInfo.CurrentCulture, "There is already a value definition item with the name {0}", item.Name), "item");
                }
        ...
        /// <exception cref="ReadOnlyException">The definition has been committed and is now read-only</exception>
        public void Clear()
        {
            lock (m_Definition.Lock)
            {
