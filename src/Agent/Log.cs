...
using AgentConfiguration = Gibraltar.Agent.Configuration.AgentConfiguration;
...
namespace Gibraltar.Agent
{
    ...
    ///         started and then manipulate the <see cref="Gibraltar.Agent.Configuration.AgentConfiguration">Agent</see> configuration
...
#pragma warning restore 169
        ...
        /// <summary>
        /// Raised whenever the Loupe Live Viewer receives a new log message, allowing the client to block it from being
        ...
        /// Loupe Live Viewer itself (accessible via hotkey unless disabled) is created within the Agent and its control
        /// object is not accessible to client code.  This static event on the Log class therefore allows clients to bind to
        /// the filter on the main Loupe Live Viewer in order to filter the messages it displays to users (e.g. to block
        ...
        /// be null to signify the main Loupe Live Viewer rather than a client-instantiated LiveLogViewer.</para></remarks>
        public static event MessageFilterEventHandler LiveViewerMessageFilter
        {
            add
            {
                ...
                {
                    if (s_MessageFilterEvent == null)
                    {
                        Listener.LiveViewerMessageFilter += LiveViewerOnMessageFilter;
                    }

                    s_MessageFilterEvent += value;
                }
            }
            remove
            {
        ...
        public static bool AutoSendConsentGranted
        {
            get
            {
                return Monitor.Log.AutoSendConsentGranted;
            }
        }
        ...
        public static void DisplayStartupConsentDialog()
        {
            ...
            Monitor.Log.DisplayStartupConsentDialog();
        }
        ...
        public static void DisplayConsentDialog()
        {
            ...
            Monitor.Log.DisplayConsentDialog();
        }
        ...
        ///         which disrupt a user activity, see the <see cref="ReportException">ReportException</see> method.
        ///     </para>
        /// 	<para>
        ...
        ///         not disrupt a user activity, see the <see cref="RecordException">RecordException</see> method.
        ///     </para>
        ...
        [MethodImplAttribute(MethodImplOptions.NoInlining)]
        public static DialogResult ReportException(Exception exception, string category, bool canContinue = true, bool waitForResponse = true)
        {
            DialogResult result = DialogResult.None;

            if (exception != null)
                result = Monitor.Log.ReportException(1, exception, null, category, canContinue, waitForResponse);
        ...
        ///         overload of <see cref="System.Windows.Forms.Application.Run()">Application.Run</see> to ensure that
        ///         the Loupe Agent registers as the event handler for <see cref="System.Windows.Forms.Application.ThreadException">Application.ThreadException</see>
        ...
        ///         Loupe (which may be through Trace), and the Agent inspects the thread's
        ///         <see cref="System.Threading.ApartmentState">ApartmentState</see> to distinguish
        ///         likely UI threads from non-UI threads. The Agent will only automatically
        ///         register for application exception events on threads created with a <see cref="System.Threading.ApartmentState.STA">
        ///             single-threaded apartment, not for threads created with the default
        ///             <see cref="System.Threading.ApartmentState.MTA">multi-threaded
        ///             apartment</see>.
        ///         </see>
        /// 	</para>
        ...
        public static void StartSession(AgentConfiguration configuration)
        {
            ...
            Monitor.Log.StartSession(configuration.WrappedObject, 1, null);
        }
        ...
        public static void StartSession(AgentConfiguration configuration, string reason)
        {
            ...
            Monitor.Log.StartSession(configuration.WrappedObject, 1, reason);
        }
        ...
        public static void StartSession(AgentConfiguration configuration, int skipFrames, string reason)
        {
            ...
            Monitor.Log.StartSession(configuration.WrappedObject, skipFrames + 1, reason);
        }
        ...
        public static void StartSession(AgentConfiguration configuration, IMessageSourceProvider sourceProvider, string reason)
        {
            ...
            Monitor.Log.StartSession(configuration.WrappedObject, (sourceProvider == null) ? null : new MessageSourceProvider(sourceProvider), reason);
        }
        ...
        /// <para>Note for users of Mono: Because the Mono runtime does not support multiple UI threads, the Loupe Live
        ...
        /// <see cref="Gibraltar.Agent.Windows.LiveLogViewer">LiveLogViewer</see>
        ...
        /// See <see cref="SmtpClient.Send(System.Net.Mail.MailMessage)">StmpClient.Send</see> for exception information.</para></remarks>
        public static void SendMessage(MailMessage message)
        {
            //get the email server from our configuration.
            EmailConfiguration mailConfig = Monitor.Log.Configuration.Email;
            PackagerConfiguration packager = Monitor.Log.Configuration.Packager;

            SmtpClient client;
            if (string.IsNullOrEmpty(mailConfig.Server))
            {
            ...
            else
            {
                ...
                {
                    client = new SmtpClient(mailConfig.Server);
                }
                else
                {
            ...
            if (string.IsNullOrEmpty(mailConfig.User) == false)
            {
            ...
            //do we need to default either the FROM or TO addresses?
            if ((message.To.Count == 0) && (string.IsNullOrEmpty(packager.DestinationEmailAddress) == false))
            {
                message.To.Add(packager.DestinationEmailAddress);
            }
           
            if (((message.From == null) || (string.IsNullOrEmpty(message.From.Address))) 
                && (string.IsNullOrEmpty(packager.FromEmailAddress) == false))
            {
                message.From = new MailAddress(packager.FromEmailAddress);
            }

            //now send our email! We want exceptions to flow back to the caller.
            client.Send(message); 
        }
        ...
        /// </remarks>
        public static bool SendSessions(SessionCriteria criteria)
        {
            //don't do jack if we aren't initialized.
            if (Monitor.Log.IsLoggingActive(false) == false)
                return false;

            return Monitor.Log.SendSessions((Gibraltar.Data.SessionCriteria)criteria, null, false);
        }
        ...
        /// consuming resources.</para>
        /// </remarks>
        public static bool SendSessions(Predicate<SessionSummary> sessionMatchPredicate)
        {
            //don't do jack if we aren't initialized.
            if (Monitor.Log.IsLoggingActive(false) == false)
                return false;
            ...
            return Monitor.Log.SendSessions(null, predicateAdapter.Predicate, false);
        }
        ...
        /// consuming resources.</para>
        /// </remarks>
        public static bool SendSessionsAsync(SessionCriteria criteria)
        {
            //don't do jack if we aren't initialized.
            if (Monitor.Log.IsLoggingActive(false) == false)
                return false;

            return Monitor.Log.SendSessions((Gibraltar.Data.SessionCriteria)criteria, null, true);
        }
        ...
        /// consuming resources.</para>
        /// </remarks>
        public static bool SendSessionsAsync(Predicate<SessionSummary> sessionMatchPredicate)
        {
            //don't do jack if we aren't initialized.
            if (Monitor.Log.IsLoggingActive(false) == false)
                return false;
            ...
            var predicateAdapter = new SessionSummaryPredicate(sessionMatchPredicate);
            return Monitor.Log.SendSessions(null, predicateAdapter.Predicate, true);
        }
        ...
        public static IServerAuthenticationProvider ServerAuthenticationProvider
        {
            ...
            set
            {
                ...
                //and set it through to the credential manager which is really what puts it into use.
                CachedCredentialsManager.DefaultAuthenticationProvider =
                    value == null ? null 
                    : new WebAuthenticationProvider(value);
            }
        }
        ...
        static void LogOnInitializing(object sender, Monitor.LogInitializingEventArgs e)
        {
            ...
            if (tempEvent != null)
            {
                ...
                LogInitializingEventArgs eventArgs = new LogInitializingEventArgs(new Configuration.AgentConfiguration(e.Configuration));
                try
                {
