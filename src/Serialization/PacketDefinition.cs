...
namespace Gibraltar.Serialization
{
    ...
    public sealed class PacketDefinition : IEnumerable<FieldDefinition>, IEquatable<PacketDefinition>
    {
        ...
        public static bool IsSerializableType(Type candidateType)
        {
            ...
            if (m_GetRequiredPacketsMethod != null)
            {
                ...
                if ((currentRequiredPackets != null) && (currentRequiredPackets.Length > 0))
                {
                    ...
                    foreach (IPacket requiredPacket in currentRequiredPackets)
                    {
                        ...
                        {
                            //we can't process this - but instead of up and failing, lets write out  a debug warn
        ...
        private void ReadFields(Type type, IPacket packet, IFieldReader reader)
        {
            ...
            if (m_BasePacket != null)
            {
                try
                {
                    m_BasePacket.ReadFields(type.BaseType, packet, reader);
                }
                catch (Exception ex)
                {
        ...
        {
            MethodInfo requestedMethod = type.GetMethod("Gibraltar.Serialization.IPacket." + methodName, flags, null, methodArgTypes, null);

            //KM:  We no longer support implementations except of the interface method
            //if (requestedMethod == null)
            //{
            //    requestedMethod = type.GetMethod(methodName, flags, null, methodArgTypes, null);
            //}
