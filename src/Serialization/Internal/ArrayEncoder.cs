...
namespace Gibraltar.Serialization.Internal
{
    ...
    /// is written as a negative value.  This avoid the waste of preceding each
    /// value with a run length of 1 as would occur in classic RLE encoding.
    /// </summary>
    ...
    internal class ArrayEncoder<T>
    {
        ...
        public ArrayEncoder()
        {
            ...
            if (m_ReadMethod == null)
                ErrorNotifier.Notify(this, new GibraltarException("Could not find method: IFieldReader." + readMethodName));

            if (m_WriteMethod == null)
            {
                ErrorNotifier.Notify(this,
                                     new GibraltarException("Could not find method: IFieldWriter.Write(" + typeof(T).Name + ")"));
            }
        }
        ...
        /// <summary>
        /// Helper method to write out a single run (either repeating or unique values)
        /// </summary>
