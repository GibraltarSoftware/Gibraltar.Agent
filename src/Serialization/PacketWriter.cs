...
namespace Gibraltar.Serialization
{
    public class PacketWriter : IPacketWriter, IDisposable
    {
        ...
        public void Write(IPacket packet)
        {
            ...
            MemoryStream encodedPayload = FieldWriter.WriteLength(payloadLength);
            var lengthLength = (int)encodedPayload.Length;

            var packetBytes = new byte[lengthLength + payloadLength];

            encodedPayload.Position = 0; // reset the position to copy into packetBytes
            encodedPayload.Read(packetBytes, 0, lengthLength);

            m_Buffer.Position = 0; // reset the position to copy into packetBytes
