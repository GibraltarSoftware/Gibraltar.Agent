...
namespace Gibraltar.Agent.Test.Packaging
{
    [TestFixture]
    public class PackagerTests
    {
        private const string EmailFromAddess = "autosender@gibraltarsoftware.com";
        private const string EmailToAddress = "Gibraltar Software@sds.gibraltarsoftware.com";
        private const string EmailOverrideServer = "smtp.sendgrid.net";
        ...
        [Test]
        public void CreateCompositeCriteriaSessionPackage()
        {
            //we need to guarantee that there is something being logged for this test to work.
            Log.StartSession("Just making sure we're up and logging for this test.");

            var criteria = SessionCriteria.CrashedSessions |
                                   SessionCriteria.WarningSessions |
                                   SessionCriteria.ActiveSession;

            using (Packager newPackager = new Packager())
            {
                File.Delete(m_OutputFileNamePath);
                newPackager.SendToFile(criteria, false, m_OutputFileNamePath);
            }
...
[Test]
        ...
        public void CreatePackageFromAlternateDirectory()
        {
            ...
            var loggingPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
        ...
        [Test]
        public void SendPackageViaEmailMissingArgs()
        {
        ...
        public void SendPackageViaEmailOverrideServer()
        {
            ...
            {
                newPackager.SendEmail(SessionCriteria.AllSessions, false, "SendPackageViaEmailOverrideServer", 
                    EmailFromAddess, EmailToAddress, EmailOverrideServer, string.Empty, string.Empty);
            }
        }
        ...
        public void SendPackageViaEmailOverrideServerAndUser()
        {
            using (Packager newPackager = new Packager())
            {
                newPackager.SendEmail(SessionCriteria.AllSessions, false, "SendPackageViaEmailOverrideServerAndUser", 
                    EmailFromAddess, EmailToAddress, EmailOverrideServer, EmailOverrideServerUser, EmailOverrideServerPassword);
            }
        }
        */
        ...
        [Ignore("Waiting on us to deploy v5 to prod to make this go live")]
        [Test]
        public void SendPackageViaServerOverrideServerAndAccountKey()
        {
            using (Packager newPackager = new Packager())
            {
                newPackager.SendToServer(SessionCriteria.AllSessions, "e98ae26495c04bf88a8af2f38286ecac", false, false, "localhost", 22454, false, null);
            }
        }
